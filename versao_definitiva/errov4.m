...
% Visualização do sinal de referência linear e do sinal amostrado digital segurado
...
% Criar representação em degraus do sinal digital
% Para visualizar corretamente em formato de degraus, precisamos de pontos duplicados
x_degraus = [];
y_degraus = [];
...
if length(amostraFaixa) > 0
    ...
    plot(5:100, sinalReferenciaLinear, 'r-', 'LineWidth', 2);
    hold on;
    ...
    title('Comparação: Sinal de Referência Linear vs. Sinal Digital Segurado');
    xlabel('Código');
    ylabel('Amplitude');
    legend('Sinal de Referência (5-100)', 'Sinal Digital Segurado', 'Pontos de Amostragem');
    grid on;
    ...
    printf("\n--- Análise de Offset: Linear vs. Digital Segurado (5-100) ---\n");
